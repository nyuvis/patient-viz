...
def parsedata(cohort):
    ...
    delimiter_csv = ','    
    try:
        csvfile = open(cohort, 'r')
        cohortsize = len(csvfile.readlines())-1
        csvfile.seek(0)
        cohortcolumns = len(csvfile.readline().split(delimiter_csv))-1
        data = np.zeros((cohortsize,cohortcolumns), dtype='bool')
        #------
        csvfile.seek(0)
        cohortreader = csv.reader(csvfile, delimiter=delimiter_csv)       
        for (ix,row) in enumerate(cohortreader):
            if (ix ==0 ):
                continue        
            data[(ix-1),:] = np.array(row[1:])        
        csvfile.close()
        #-----
        # right now depends on the location of the test column and output column. 
        # could be improved in terms of software-engineering aspect, and get the column from CSV header
        test_ix = data[:,0]            
        ...
        print('cohort input does not look like csv file. reading returned error', file=sys.stderr)
        exit(1)    
...
def buildmodel(cohort, model, validPercentage, seed, modeloutput):    
    ...
    if model == 'SVM' or model == 'randForest':
        print ( model, ' model will be implemented later.', file=sys.stderr)
        exit(1)

def usage():
    print('usage: {0} [-h] --i <input cohort file> --mo <model output> [--v <validation size>] [--seed <seed>] [--model <model type>]'.format(sys.argv[0]), file=sys.stderr)
    print('-h: print help', file=sys.stderr)
    print('--i <cohort file>: specifies the full cohort', file=sys.stderr)
    print('--odir <output-dir>: specifies model output directory.', file=sys.stderr)
...
if __name__ == '__main__':
    ...
    while args:
        ...
        if arg == '--i':
            if not len(args):
                print('--i requires cohort file ', file=sys.stderr)
                usage()
            cohort = args.pop(0)        
        elif arg == '--odir':
            if not len(args):
                print('--odir requires model output directory', file=sys.stderr)
                usage()
    ...
    if not len(cohort):
        print('requires input full cohort')
        usage()
